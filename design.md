...
- Motion Stance: **Motion layer** (user-requested). Slow, cinematic cadence: hover micro-interactions (lift, teal glow, sheen sweep) run at 340–600ms; scroll reveals are **item-level** (cards, workflow rows, timeline entries, text blocks — grids cascade through to their cards rather than moving as one slab) via IntersectionObserver — `opacity` and `transform` only, ~1350ms with per-batch staggered delays ≤1200ms (200ms step, applied only to elements entering together). Hero load-in staggers at 280ms steps over ~2.7s. Marquee 52s, beacon pulse 4.5s. Molecule canvas drifts at 0.6× speed with eased pointer parallax. No scroll parallax or aurora blobs.
...
- **Nav**: **N5 Floating Pill** — content-sized, detached from edges, solid obsidian/light surface with 1px hairline border and subtle shadow. Drops into place on load, with 2px extra bottom clearance (pill `margin-bottom` + 74px hero top padding). Responsive: full link row from 1024px (tightened spacing ≤1160px, `nowrap` links, pill capped at `max-width: 100%`); below 1024px the pill spans the full gutter width (brand left, actions right, `space-between`) with hidden-scrollbar horizontal overflow scrolling as a safety net, and the links collapse into an animated drawer (fade/slide/scale entrance, dimmed backdrop, teal accent border when open, 50ms-per-link staggered cascade, thin styled scrollbar + momentum touch scrolling, closes on backdrop click, link tap, Escape, or resize-to-desktop, html+body scroll locked while open) with a 40px CSS-only hamburger that morphs bar-by-bar into an ×. On mobile the pill tucks above the viewport while scrolling down and slides back on any scroll up (suppressed through anchor jumps); desktop never tucks. Tap targets ≥36px. Media-query display/gap overrides use `!important` to out-rank Tailwind's `.flex`/`gap-1` utilities, which the CDN injects later at equal specificity and would otherwise keep the desktop link row visible on phones.
